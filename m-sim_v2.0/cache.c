...
reset_cache_stats(struct cache_t *cp){
  ...
  cp->bus_free = 0;
  /* restet replacement delays for each block */
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if ((addr + nbytes) > ((addr & ~cp->blk_mask) + cp->bsize))
    fatal("cache: access error: access spans block, addr 0x%08x (DEBUG_cepoiu: cache type [%s] cache bsize [%d] cache mask [%d])", addr, cp->name, cp->bsize, cp->blk_mask);
